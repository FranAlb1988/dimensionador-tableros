      ...
      >
        <header className="px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
          ...
          <button
            onClick={onCerrar}
            aria-label="Cerrar"
            className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 text-xl leading-none"
          >
            ×
          </button>
        </header>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <section className="grid grid-cols-2 gap-3 text-sm">
            <label className="flex flex-col gap-1">
              <span className="text-slate-500 dark:text-slate-400">Tensión primaria (MT)</span>
              <select
            ...
            <label className="flex flex-col gap-1">
              <span className="text-slate-500 dark:text-slate-400">Tensión secundaria (BT)</span>
              <select
            ...
            <label className="flex flex-col gap-1 col-span-2">
              <span className="text-slate-500 dark:text-slate-400">Tipo constructivo</span>
              <div
            ...
            <label className="flex flex-col gap-1 col-span-2">
              <span className="text-slate-500 dark:text-slate-400">Margen de crecimiento</span>
          ...
          <section className="border-t border-slate-200 dark:border-slate-800 pt-3">
            ...
            <dl className="grid grid-cols-2 gap-y-1 text-sm">
              <dt className="text-slate-500 dark:text-slate-400">Carga del CDC</dt>
              <dd className="font-medium tabular-nums">{fmtAmp(corrienteTotalA)}</dd>

              <dt className="text-slate-500 dark:text-slate-400">Potencia requerida</dt>
              <dd className="font-medium tabular-nums">
                {resultado.kvaRequerido.toFixed(0)} kVA
              </dd>

              <dt className="text-slate-500 dark:text-slate-400">Potencia nominal sugerida</dt>
              <dd
                className={
              ...
              <dt className="text-slate-500 dark:text-slate-400">Primario {cfg.tensionPrimariaKv} kV</dt>
              <dd className="font-medium tabular-nums">{fmtAmp(resultado.inPrimarioA)}</dd>

              <dt className="text-slate-500 dark:text-slate-400">Secundario {cfg.tensionSecundariaV} V</dt>
              ...
              <dt className="text-slate-500 dark:text-slate-400">Impedancia (Ucc)</dt>
              <dd className="font-medium tabular-nums">{resultado.uccPorcentaje} %</dd>

              <dt className="text-slate-500 dark:text-slate-400">Icc secundario (red infinita)</dt>
              ...
              <dt className="text-slate-500 dark:text-slate-400">Pérdidas en vacío (P₀)</dt>
              <dd className="font-medium tabular-nums">{resultado.perdidasVacioW} W</dd>

              <dt className="text-slate-500 dark:text-slate-400">Pérdidas en carga (Pk)</dt>
